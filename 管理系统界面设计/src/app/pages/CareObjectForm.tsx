...
const CARE_TYPE_OPTIONS = ["孤寡老人", "留守儿童", "留守妇女", "其他"];
...
export function CareObjectForm() {
  ...
  useEffect(() => {
    ...
    fetchJson<any>(`/api/care-objects/${id}/`)
      .then((data) => {
        setForm({
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (id) {
        await putJson(`/api/care-objects/${id}/`, { resident_id: selectedResident.id, ...form });
        setMessage("关爱对象信息已更新。");
      } else {
        await postJson("/api/care-objects/create/", { resident_id: selectedResident.id, ...form });
        setMessage("关爱对象信息已保存。");
      }
      setTimeout(() => navigate("/care-objects"), 800);
    } catch (error) {
      setErrorMessage((error as Error).message);
    } finally {
      setSubmitting(false);
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded border bg-white px-6 py-5" style={{ borderColor: "#e4e7ed" }}>
        ...
        <section>
          <SectionTitle title="关爱信息" />
          <div className="grid grid-cols-4 gap-4">
            <div>
              <FieldLabel text="关爱类型" />
              <SelectField value={form.care_type} onChange={(value) => setField("care_type", value)} options={CARE_TYPE_OPTIONS} placeholder="请选择关爱类型" disabled={isDetail} />
            </div>
            <div>
              <FieldLabel text="关爱等级" />
              <TextInput value={form.care_level} onChange={(value) => setField("care_level", value)} placeholder="请输入关爱等级" disabled={isDetail} />
            </div>
            <div>
              <FieldLabel text="关爱人员" />
              <TextInput value={form.caregiver_name} onChange={(value) => setField("caregiver_name", value)} placeholder="请输入关爱人员" disabled={isDetail} />
            </div>
            <div>
              <FieldLabel text="联系方式" />
              <TextInput value={form.caregiver_phone} onChange={(value) => setField("caregiver_phone", value)} placeholder="请输入联系方式" disabled={isDetail} />
            </div>
          </div>
