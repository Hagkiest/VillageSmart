...
type FormState = {
  ...
  required_attendance_days: string;
...
export function PublicJobForm() {
  ...
  useEffect(() => {
    ...
    fetchJson<any>(`/api/public-jobs/${id}/`)
      .then((data) => {
        setForm({
          ...
          required_attendance_days: String(data.required_attendance_days ?? 0),
          actual_attendance_days: String(data.actual_attendance_days ?? 0),
          status: data.status || "在岗",
          notes: data.notes || "",
        });
        setSelectedResident({
          id: data.resident_id,
  ...
  const residentSummary = useMemo(
    () => [
      ...
      { label: "户属性", value: selectedResident?.household_type ?? "" },
    ],
    [selectedResident]
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (id) {
        await putJson(`/api/public-jobs/${id}/`, { resident_id: selectedResident.id, ...form });
        setMessage("公益性岗位信息已更新。");
      } else {
        await postJson("/api/public-jobs/create/", { resident_id: selectedResident.id, ...form });
        setMessage("公益性岗位信息已保存。");
      }
      setTimeout(() => navigate("/public-jobs"), 800);
    } catch (error) {
      setErrorMessage((error as Error).message);
    } finally {
      setSubmitting(false);
    ...
    <div className="space-y-3">
      ...
      <div className="rounded border bg-white px-6 py-5" style={{ borderColor: "#e4e7ed" }}>
        ...
        <section>
          ...
          <div className="grid grid-cols-4 gap-4">
            <div>
              <FieldLabel text="岗位名称" required />
              <TextInput value={form.job_name} onChange={(value) => setField("job_name", value)} placeholder="请输入岗位名称" disabled={isDetail} />
            </div>
            <div>
              <FieldLabel text="主管部门" />
              <TextInput value={form.department} onChange={(value) => setField("department", value)} placeholder="请输入主管部门" disabled={isDetail} />
            </div>
            <div>
              <FieldLabel text="合同开始日期" />
              <TextInput value={form.start_date} onChange={(value) => setField("start_date", value)} type="date" disabled={isDetail} />
            </div>
            <div>
              <FieldLabel text="合同结束日期" />
              <TextInput value={form.end_date} onChange={(value) => setField("end_date", value)} type="date" disabled={isDetail} />
            </div>
            <div>
              <FieldLabel text="月补贴标准" />
              <TextInput value={form.subsidy_amount} onChange={(value) => setField("subsidy_amount", value)} type="number" disabled={isDetail} />
            </div>
            <div>
              <FieldLabel text="规定出勤天数" />
              <TextInput value={form.required_attendance_days} onChange={(value) => setField("required_attendance_days", value)} type="number" disabled={isDetail} />
            </div>
            <div>
              <FieldLabel text="实际出勤天数" />
              <TextInput value={form.actual_attendance_days} onChange={(value) => setField("actual_attendance_days", value)} type="number" disabled={isDetail} />
            </div>
            <div>
              <FieldLabel text="状态" />
